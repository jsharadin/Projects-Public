...
class HarvesterAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 1
        "*** YOUR CODE HERE ***"
        #inherits health instance attribute from insect class
        gamestate.food += 1
        # END Problem 1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        # BEGIN Problem 3 and 4, reference lower and upper bound
        position = self.place
        count = 0
        while position.entrance is not None:
            if not position.is_hive:
                if len(position.bees) > 0 and count >= self.lower_bound and count <= self.upper_bound:
                    return random_bee(position.bees)  # REPLACE THIS LINE
...
class HungryAnt(Ant):
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3 #class attribute
    implemented = True

    def __init__(self, health = 1):
        Ant.__init__(self, health)
        self.turns_to_chew = 0 #instance attribute
